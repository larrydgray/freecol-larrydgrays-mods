...
public class Player extends FreeColGameObject implements Nameable {
    ...
    /** LarryDGray's Mods: running totals of gold gained/lost so far
     *  this turn, accumulated by {@link #modifyGold} - not persisted
     *  directly, just read and reset once per turn by
     *  {@code ServerPlayer.csNewTurn()} to build a {@code
     *  GoldJournalSample} for the Gold Journal report. */
    private int goldInThisTurn = 0;
    private int goldOutThisTurn = 0;
    private final java.util.Map<GoldCategory, Integer> goldInByCategoryThisTurn = new java.util.EnumMap<>(GoldCategory.class);
    private final java.util.Map<GoldCategory, Integer> goldOutByCategoryThisTurn = new java.util.EnumMap<>(GoldCategory.class);
    ...
    /** LarryDGray's Mods: this player's own turn-by-turn Gold Journal
     *  report history - how much gold was gained/lost each turn and
     *  the resulting balance, so it survives a save/reload instead of
     *  living only in memory for the session. */
    protected final List<GoldJournalSample> goldJournalHistory = new ArrayList<>();
    ...
    /**
     * LarryDGray's Mods: an iterator for the player units currently
     * following an Auto Explore order.
     */
    private final UnitIterator nextAutoExploreUnitIterator
        = new UnitIterator(this, Unit::autoExploringUnit);
     ...
     *
     * LarryDGray's Mods: prefer {@link #modifyGold(int, GoldCategory)}
     * at any new call site - this overload just tags the change as
     * {@link GoldCategory#OTHER} for the Gold Journal report.
     *
    ...
    public int modifyGold(int amount) {
        return modifyGold(amount, GoldCategory.OTHER);
    }

    /**
     * LarryDGray's Mods: modifies the amount of gold that this player
     * has, tagging the reason for the Gold Journal report. Every gold
     * change funnels through this one method regardless of the
     * reason (trade, upkeep, treasure, tribute, etc.), so accumulating
     * per-category totals here means the report always reflects every
     * gold-affecting call site, including any added in the future
     * (they'll just show up as "Other" until tagged).
     *
     * @param amount The amount of gold to be added to this player.
     * @param category The {@code GoldCategory} responsible for the
     *     change.
     * @return The amount of gold post-modification.
     */
    public int modifyGold(int amount, GoldCategory category) {
        if (amount > 0) {
            this.goldInThisTurn += amount;
            this.goldInByCategoryThisTurn.merge(category, amount, Integer::sum);
        } else if (amount < 0) {
            this.goldOutThisTurn += -amount;
            this.goldOutByCategoryThisTurn.merge(category, -amount, Integer::sum);
        }
    ...
    public boolean removeUnit(final Unit oldUnit) {
        ...
        nextAutoExploreUnitIterator.remove(oldUnit);
    ...
    /**
     * LarryDGray's Mods: get a new Auto Explore unit.
     *
     * @return A {@code Unit} that can be processed by the Auto
     *     Explore per-turn driver.
     */
    public Unit getNextAutoExploreUnit() {
        return nextAutoExploreUnitIterator.next();
    }

    /**
     * LarryDGray's Mods: checks if there is a unit currently
     * following an Auto Explore order.
     *
     * @return True if there is such a unit.
     */
    public boolean hasNextAutoExploreUnit() {
        return nextAutoExploreUnitIterator.hasNext();
    }
    ...
    public void resetIterators() {
        ...
        nextAutoExploreUnitIterator.reset();
    }
    ...
    /**
     * LarryDGray's Mods: get this player's Gold Journal report
     * history.
     *
     * @return A copy of the {@code GoldJournalSample}s for this
     *     player, oldest first.
     */
    public final List<GoldJournalSample> getGoldJournalHistory() {
        synchronized (this.goldJournalHistory) {
            return new ArrayList<>(this.goldJournalHistory);
        }
    }

    /**
     * LarryDGray's Mods: append a Gold Journal sample for this
     * player, trimming the oldest once the cap is exceeded.
     *
     * @param sample The {@code GoldJournalSample} to add.
     */
    public void addGoldJournalSample(GoldJournalSample sample) {
        synchronized (this.goldJournalHistory) {
            this.goldJournalHistory.add(sample);
            if (this.goldJournalHistory.size() > MAX_HISTORY_SAMPLES) {
                this.goldJournalHistory.remove(0);
            }
        }
    }

    /**
     * LarryDGray's Mods: forget this player's Gold Journal report
     * history, e.g. when a different game is connected.
     */
    public void clearGoldJournalHistory() {
        synchronized (this.goldJournalHistory) {
    ...
    /**
     * LarryDGray's Mods: get the gold gained so far this turn, for
     * the Gold Journal report.
     *
     * @return The gold gained this turn.
     */
    public final int getGoldInThisTurn() {
        return this.goldInThisTurn;
    }

    /**
     * LarryDGray's Mods: get the gold lost so far this turn, for the
     * Gold Journal report.
     *
     * @return The gold lost this turn.
     */
    public final int getGoldOutThisTurn() {
        return this.goldOutThisTurn;
    }

    /**
     * LarryDGray's Mods: get the gold gained so far this turn, broken
     * down by category, for the Gold Journal report.
     *
     * @return The gold gained this turn, by category.
     */
    public final java.util.Map<GoldCategory, Integer> getGoldInByCategoryThisTurn() {
        return this.goldInByCategoryThisTurn;
    }

    /**
     * LarryDGray's Mods: get the gold lost so far this turn, broken
     * down by category, for the Gold Journal report.
     *
     * @return The gold lost this turn, by category.
     */
    public final java.util.Map<GoldCategory, Integer> getGoldOutByCategoryThisTurn() {
        return this.goldOutByCategoryThisTurn;
    }

    /**
     * LarryDGray's Mods: reset the running gold in/out totals, once a
     * turn's {@code GoldJournalSample} has been built from them.
     */
    public void resetGoldJournalAccumulators() {
        this.goldInThisTurn = 0;
        this.goldOutThisTurn = 0;
        this.goldInByCategoryThisTurn.clear();
        this.goldOutByCategoryThisTurn.clear();
    }
    ...
    public static enum NoClaimReason implements Named {
        ...
        TOO_CLOSE;       // LarryDGray's Mods: too close to one of our other colonies
    ...
    public NoClaimReason canClaimToFoundSettlementReason(Tile tile) {
        if (!tile.getType().canSettle()) return NoClaimReason.TERRAIN;
        NoClaimReason reason = canClaimForSettlementReason(tile);
        if (reason == NoClaimReason.NATIVES) {
            if (!tile.getAdjacentColonies().isEmpty()) {
                return NoClaimReason.SETTLEMENT;
            }
            reason = (canClaimFreeCenterTile(tile))
                ? NoClaimReason.NONE : NoClaimReason.NATIVES;
        }
        if (reason != NoClaimReason.NONE) return reason;
        // LarryDGray's Mods: everything else says this tile is
        // otherwise foundable - apply the minimum-colony-distance
        // rule last, so it only ever kicks in for a genuinely
        // available site.
        return tooCloseToOwnColonyReason(tile);
    }

    /**
     * LarryDGray's Mods: is the given tile too close to one of this
     * player's own existing colonies, per the
     * {@code GameOptions.MINIMUM_COLONY_DISTANCE} game option?
     * Guarded with {@code hasOption} since this is a new spec option
     * absent from continued saves made before it existed - those
     * games simply see no restriction, matching pre-existing
     * behaviour.
     *
     * @param tile The {@code Tile} to check.
     * @return {@code NoClaimReason.TOO_CLOSE} if too close, else
     *     {@code NoClaimReason.NONE}.
     */
    private NoClaimReason tooCloseToOwnColonyReason(Tile tile) {
        final Specification spec = getSpecification();
        if (!spec.hasOption(GameOptions.MINIMUM_COLONY_DISTANCE,
                            IntegerOption.class)) {
            return NoClaimReason.NONE;
        }
        final int minDistance
            = spec.getInteger(GameOptions.MINIMUM_COLONY_DISTANCE);
        if (minDistance <= 1) return NoClaimReason.NONE;
        for (Colony c : getColonyList()) {
            if (tile.getDistanceTo(c.getTile()) < minDistance) {
                return NoClaimReason.TOO_CLOSE;
            }
        }
        return NoClaimReason.NONE;
    }
    ...
    */
    ...
    protected void writeChildren(FreeColXMLWriter xw) throws XMLStreamException {
        ...
        if (xw.validFor(this)) {
            ...
            for (GoldJournalSample sample : getGoldJournalHistory()) {
                sample.toXML(xw);
            }
    ...
    protected void readChild(FreeColXMLReader xr) throws XMLStreamException {
        ...
        if (BAN_MISSIONS_TAG.equals(tag)) {
            ...
            addGoldJournalSample(new GoldJournalSample(xr));
